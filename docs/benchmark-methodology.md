# DeepSeek-V4-Flash 推理选型测试方案（深度技术文档）

> 本文是本仓库最初的内部选型测试工作文档，从根目录 [README.md](../README.md) 迁移而来。
> 它记录了完整的选型方法论、拓扑对比、硬件候选、测试矩阵与客户验收标准，技术内容与数字均保持原样，未做删改。
> 面向读者：需要理解「为什么这样测、测什么、验收线在哪」的工程/选型同学。
>
