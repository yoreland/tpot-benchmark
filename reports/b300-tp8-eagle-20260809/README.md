...
### 5.2 已补上的 H200 对比，以及本节数据的两个已知问题

H200 的同口径并发扫描已于 2026-08-10 补测完成，见
`reports/h200-tp4-eagle-20260809/README.md` 第 5 节（run `20260810-015049-2b75`）。
对比时请注意本节数据有**两个已知问题**，都是后来核对原始 JSON 才发现的：

**问题 1：本节扫描与第 4 节的 40K/30K 数字不是同一个 server 跑出来的。**
`concurrency-sweep/bench_c*.json` 里 `server_info.version` = **0.5.12.post1**，
而 `bench_custom.json` / `bench_official.json` 里是 **0.5.17**。
第 2.2 节和 `run_metadata.json` 声称全程用 `v0.5.12.post1-cu130`，与后者矛盾。
进一步的证据链：`docs/run-summaries/b300-matrix-20260809-c1-summary.md` 记录
0.5.12.post1 在这台机器上有严重缺陷（≥16K 输入会把全部 8 个 TP rank 打崩），
其 4000/1500 实测 TPOT P50 = 4.503 ms，与本节 c=1 的 4.51 ms 几乎相同 ——
说明**本节的低并发数字是被旧版本压低的，不代表 B300 硬件能力**。
因此上面「c=1 只有 161.7 tok/s」不能拿去和 H200 的 286.2 tok/s 比硬件。

**问题 2：每档只有 32 条 prompts，高并发档没进稳态。**
c=32 那档 duration 只有 9.99 s、实测并发只到 19.95（32 条请求一次性灌完就结束）。
H200 侧做了 32 vs 128 prompts 的对照实验（见那份报告 5.4）：同样是 c=32，
样本量从 32 加到 128 之后 TPOT P50 上涨 65%、E2E P50 上涨 60%。
所以**本节 c=16 / c=32 的延迟数字应当理解为乐观值**，
上面「c=32 的 TPOT 异常低（6.29 ms < c=16 的 7.43 ms）」大概率就是这个原因，
而不是「scheduling 策略在高并发下更激进」。

要得到可用于硬件选型的 B300 数字，需要在 **0.5.17** 上、用**加大样本量**的口径重跑本节。
...
- [x] 在 H200 上补充并发扫描测试 (8K/1.5K, c=1~32) 进行直接对比 ——
      已完成，见 `reports/h200-tp4-eagle-20260809/README.md` 第 5 节
- [ ] **在 SGLang 0.5.17 上重跑本报告第 5 节的并发扫描**，并把每档样本量加大到 128：
      现有扫描跑在 0.5.12.post1 上且每档只有 32 条 prompts，低并发被旧版本压低、
      高并发延迟偏乐观（详见 5.2），无法用于跨硬件定量对比
